...
/**
 * Handler: BSE Students
 *
 * GET    /api/bse-students           — list all BSE students       (admin only)
 * GET    /api/bse-students/{id}     — get single student        (admin only)
 * POST   /api/bse-students          — create student          (admin only)
 * PATCH  /api/bse-students/{id}     — update student fields   (admin only)
 * DELETE /api/bse-students/{id}     — delete student          (admin only)
 */

function handleBseStudents(PDO $pdo, string $method, ?int $id, array $body): void
{
    requireAdmin($pdo);
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO bse_students (student_id, name, program, year_level, gmail, school_year_id, semester)
        ...
        case 'PATCH':
            ...
            $sql = 'UPDATE bse_students SET ' . implode(', ', $fields) . ' WHERE id = :id';
        ...
        case 'DELETE':
            ...
            $stmt = $pdo->prepare('DELETE FROM bse_students WHERE id = :id');
            $stmt->execute([':id' => $id]);
            $stmt->rowCount()
